...
shap_values = explainer(X_scaler)
...
for i in range(1, len(shap_summary) + 1):
    ...
    select_X_all = X_scaler[:, feature_indices]
    ...
    cv_scores = cross_val_score(model, select_X_all, y, cv=cv_strategy, scoring='r2')###
    cv_r2_mean = cv_scores.mean()
    cv_mse_scores = cross_val_score(model, select_X_all, y, cv=cv_strategy, scoring='neg_mean_squared_error')
