...
#use the best_params from XGBoost_opt.py
learning_rate=0.1
n_estimators=200
max_depth=7
min_child_weight=5
...
cv_r2 = cross_val_score(xg_reg, X_scaler, y, cv=cv_strategy, scoring='r2').mean()
print(f"Cross-Validation R-squared: {cv_r2}")
cv_scores = cross_val_score(xg_reg, X_scaler, y, cv=cv_strategy, scoring='r2')
print(f"Cross-Validation R-squared scores on entire data: {cv_scores}")

cv_mse_scores = -cross_val_score(xg_reg, X_scaler, y, cv=cv_strategy, scoring='neg_mean_squared_error')
...
xg_reg.save_model(r'yourpath\xgboost_model.json')
import pickle
# Save LabelEncoders and Scaler
with open(r'yourpath\xgboost_scaler.pkl', 'wb') as f:
...
with open(r'yourpath\xgboost_data.pkl', 'wb') as f:
